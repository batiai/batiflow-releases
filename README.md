...
카카오톡 · Slack · iMessage — 메시지 발송과 읽기를 자동화하세요
...
📨 **메시지 읽기** — 카카오톡 채팅방 메시지 자동 파싱 + SQLite 저장 + 중복 제거
...
🔍 **스마트 검색** — 정확 일치 우선 정렬 + 정확히 일치 필터 옵션
...
> 💡 **접근성 권한이 없으면** 메시지 발송, 읽기, MCP 모두 동작하지 않습니다.
...
- **읽기** — 채팅방 선택 → 메시지 자동 파싱 → 저장된 대화 탭에서 재조회
- **주소록** — 카톡 친구 가져오기, 그룹 관리, CSV 내보내기
- **가이드** — 앱별 사용법, CLI 명령어, 이용 안내
- **설정** — MCP 설정, 발송 속도 조절, 데이터 초기화, 상태 확인
...
# 채팅 목록 보기
batiflow-cli chats --limit 20

# Slack (DM)
batiflow-cli send "류바티" "확인 부탁드립니다" --app slack
...
BatiFlow는 MCP(Model Context Protocol)를 지원하여 Claude Code나 Cursor에서 자연어로 메시지를 보내고 읽을 수 있습니다.
...
| 앱 | 메시지 발송 | 메시지 읽기 | 채팅 목록 | 친구 목록 | 자동화 방식 |
|----|:---------:|:---------:|:--------:|:--------:|-----------|
| 카카오톡 | ✅ | ✅ | ✅ | ✅ | AX API |
...
2. **강제 종료** 버튼으로 앱을 완전히 종료할 수 있습니다
3. 연속 3회 실패 시 자동으로 중단됩니다
4. 카카오톡이 **로그인 상태**인지 확인하세요
5. 카카오톡 **비밀번호 잠금**이 설정되어 있으면 해제하세요
6. 설정에서 **발송 속도**를 "느림"으로 조절해보세요
7. 설정 > **데이터 관리**에서 초기화 후 재시도
</details>
